 ...
 * 拍子
 */
JNIEXPORT void JNICALL Java_com_zhidaoauto_lib_soundtouch_SoundTouch_setTempo
...
JNIEXPORT void JNICALL Java_com_amoment_audio_JNISoundTouch_putSamples
        (JNIEnv *env, jobject obj, jfloatArray samples, jint len) {
// 转换为本地数组
    jfloat *input_samples = env->GetFloatArrayElements(samples, JNI_FALSE);
   // mSoundTouch.putSamples(input_samples, len);
    // 释放本地数组(避免内存泄露)
    env->ReleaseFloatArrayElements(samples, input_samples, 0);
}


JNIEXPORT jfloatArray JNICALL Java_com_amoment_audio_JNISoundTouch_receiveSamples
        (JNIEnv *env, jobject obj) {
    float buffer[BUFFER_SIZE];
    int nSamples = mSoundTouch.receiveSamples(buffer, BUFFER_SIZE);

    // 局部引用，创建一个short数组
    jfloatArray receiveSamples = env->NewFloatArray(nSamples);
    // 给short数组设置值
    env->SetFloatArrayRegion(receiveSamples, 0, nSamples, buffer);
